...
// nothing to validate if the values were posted, only inserting forign keys so changing the hidden field will only allow the user to insert otherwise legal values (e.g the id of a different, extant, track)

if (isset($_POST['submit']))
{  
  $stmt = $db->prepare("UPDATE user SET fav_track=? WHERE username=?");
  $result = $stmt->execute( [$_POST['track_id'], $_SESSION['uname']]);
